...
function plot_res(result, powergrid,disturbed_node)
    ...
    append!(ω_indices,findall(n -> isa(n, ThirdOrderEqExciterIEEED1A), powergrid.nodes))
    #append!(ω_indices,findall(n -> isa(n, ThirdOrderGovExc), powergrid.nodes))

    ω_colors = reshape(Plots.get_color_palette(:auto, plot_color(:blue), 20)[ω_indices], (1,length(ω_indices)))
